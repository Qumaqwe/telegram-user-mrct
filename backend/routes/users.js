  ...
  try {
    ...
    if (existing) {
      db.get('users').find({ telegram_id: id }).assign({
        username: username || null, first_name: first_name || null, last_name: last_name || null,
      }).write();
    } else {
      db.get('users').push({
        telegram_id: id, username: username || null, first_name: first_name || null,
        last_name: last_name || null, created_at: new Date().toISOString(),
      }).write();
    }
...
router.get('/me', validateTelegramData, (req, res) => {
  ...
  const listings = db.get('listings').filter({ seller_id: id })
    .sortBy((l) => -new Date(l.created_at)).value();

  const purchases = db.get('transactions').filter({ buyer_id: id }).value().map((t) => {
